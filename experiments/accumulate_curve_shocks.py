...
file = pathlib.Path(
    r"/Users/samrullo/programming/pyprojects/fi_djrest_project/data/stress_scenario_usd_shift_01_updated.csv"
)
df = pd.read_csv(file)
periodzerodf = df[df["period_number"] == 0]
starting_curve_shocks = {
    year: shock
    for year, shock in zip(
        periodzerodf["curve_year"], periodzerodf["parallel_shock_size"]
    )
}
...
for period in range(1, 10):
    ...
    curve_shocks = {
        year: shock
        for year, shock in zip(perioddf["curve_year"], perioddf["parallel_shock_size"])
    }
    cumulative_curve_shocks = {
        year: shock + curve_shocks[year]
        for year, shock in cumulative_curve_shocks.items()
    }
    for year, shock in cumulative_curve_shocks.items():
        df.loc[
            (df["period_number"] == period) & (df["curve_year"] == year),
            "parallel_shock_size",
        ] = shock
